# compare / compare_standard 运行结果总结（1~2s 随机训练）
...
- `dl_model/compare_standard/output_official/results_aggregate_1to2s.csv`（2026-04-19 14:48:00）
- `dl_model/compare_standard/output_official/results_test_duration_aggregate_1to2s.csv`（2026-04-19 14:48:00）
...
## 2. compare_standard 总体结果（主测试口径=1.5s）
...
结论：
- `compare_standard` 里按 F1 与 ACC 都是 `ecapatdnn` 最优。
...
### compare_standard：每个时长的最佳模型（按 F1）
...
## 4. 两组公共模型对比（compare - compare_standard，主口径1.5s）

| 模型 | ΔACC | ΔF1 | 训练耗时比（standard/compare） |
|---|---:|---:|---:|
| ecapatdnn | -0.0743 | -0.0402 | 1.78x |
| tdnn | +0.0383 | -0.0479 | 0.60x |

解读：
- `ecapatdnn`：compare_standard 的精度更高，但 compare 训练更快。
- `tdnn`：compare 的 ACC 更高，但 compare_standard 的 F1 更高，且 compare 更慢。

## 5. 可直接写论文的简要结论

- 在本次 `1~2s` 随机训练设置下，`compare` 体系中 `sincnet` 综合最优，且在 `1.0/1.5/2.0s` 三个测试时长都保持第一。
- `compare_standard` 体系中 `ecapatdnn` 综合最优，并在三个测试时长下均表现稳定。
