...
from dl_model.compare.shared import SincNetPairStudent, set_seed
...
MD_COMPARE_EXPERIMENTS = {
    "c1_compare_baseline": {
        "runs_csv": "dl_model/compare/output/summary_manifest_runs.csv",
        "swap_val_test": False,
        "half_durations": [2.0],
    },
    "c2_compare_1to2s": {
        "runs_csv": "dl_model/compare/output/summary_manifest_runs_1to2s.csv",
        "swap_val_test": False,
        "half_durations": [1.0, 1.5, 2.0],
    },
    "c3_compare_change_val_test_train": {
        "runs_csv": "dl_model/compare/output/summary_manifest_runs_change_val_test_train.csv",
        "swap_val_test": True,
        "half_durations": [2.0],
    },
}
...
def resolve_experiments(args):
    ...
    names = args.experiments or list(MD_COMPARE_EXPERIMENTS.keys())
    out = []
    for name in names:
        if name not in MD_COMPARE_EXPERIMENTS:
            raise ValueError(f"Unknown experiment: {name}")
        config = dict(MD_COMPARE_EXPERIMENTS[name])
        config["name"] = name
        out.append(config)
    return out
...
def main():
    parser = argparse.ArgumentParser(
        description=(
            "Evaluate compare checkpoints and output EER only. "
            "Default mode runs all compare experiments listed in md summary (C1/C2/C3)."
        )
    )
    parser.add_argument("--mode", default="md_all", choices=["md_all", "single"])
    parser.add_argument("--experiments", nargs="+", default=None, choices=list(MD_COMPARE_EXPERIMENTS.keys()))
    ...
    parser.add_argument("--swap-val-test", action="store_true", default=False)
    ...
    parser.add_argument("--output-csv", default="dl_model/compare/output/eer_compare_md_all.csv")
    ...
    if not splits["train"] or not splits["val"] or not splits["test"]:
        raise RuntimeError(
            f"Empty split found: train={len(splits['train'])}, val={len(splits['val'])}, test={len(splits['test'])}"
        )
    ...
    for exp in experiments:
        ...
        split_samples = splits["val"] if exp["swap_val_test"] else splits["test"]
