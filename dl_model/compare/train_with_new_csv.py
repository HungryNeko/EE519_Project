...
def train_one_model(model_name, args, train_samples, test_samples, device, run_index):
    ...
    if best_acc_record:
        with open(best_acc_record_path, "w", encoding="utf-8") as f:
            json.dump(best_acc_record, f, indent=2)
    if best_f1_record:
...
def main():
    parser = argparse.ArgumentParser(description="Train with new CSV method using JSON-derived timestamps.")
    parser.add_argument("--models", nargs="+", default=list(MODEL_MODULES.keys()), choices=list(MODEL_MODULES.keys()))
    parser.add_argument("--old-csv", default="dl_model/csv2/baseline_train_test_segments.csv")
    parser.add_argument("--old-train-audio-dir", default="datasets/train_test2/train")
    parser.add_argument("--old-test-audio-dir", default="datasets/train_test2/test")
    ...
    parser.add_argument("--half-duration", type=float, default=4.0, help="Half duration in seconds for audio segments (e.g., 1.0 for ±1s, 2.0 for ±2s)")
    ...
    with open(root / args.summary_path, "w", encoding="utf-8") as f:
        json.dump(summary_rows, f, indent=2)

    summary_csv_path = root / args.summary_path.replace(".json", ".csv")
    write_csv(summary_csv_path, summary_rows)

    aggregate_csv_path = root / args.summary_path.replace(".json", "_aggregate.csv")
    write_csv(aggregate_csv_path, aggregate_rows)

    print(f"\nTraining complete. Summary saved to {root / args.summary_path}")
